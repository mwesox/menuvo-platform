...
export const hoursRouter = router({
	...
	delete: storeOwnerProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			// Verify the store belongs to the authenticated user
			if (hourEntry.storeId !== ctx.storeId) {
				throw new TRPCError({
					code: "FORBIDDEN",
			...
			await ctx.db
				.delete(storeHours)
				.where(
					and(eq(storeHours.id, input.id), eq(storeHours.storeId, ctx.storeId)),
				);
