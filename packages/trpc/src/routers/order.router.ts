...
export const orderRouter = router({
		...
		.query(async ({ ctx, input }) => {
			...
			const store = await ctx.db.query.stores.findFirst({
				where: and(
					...
					eq(stores.merchantId, ctx.merchantId), // Must match user's store
				),
				columns: { id: true },
			});
	...
	updateStatus: storeOwnerProcedure
		...
		.mutation(async ({ ctx, input }) => {
			// Verify the order belongs to a store owned by the user's merchant
			const order = await ctx.db.query.orders.findFirst({
				where: eq(orders.id, input.orderId),
			...
			if (order.store.merchantId !== ctx.merchantId) {
				throw new TRPCError({
					code: "FORBIDDEN",
