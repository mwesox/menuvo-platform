...
export const itemRouter = router({
	...
	create: storeOwnerProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!category || category.store.merchantId !== ctx.merchantId) {
				throw new TRPCError({
					code: "NOT_FOUND",
	...
	update: storeOwnerProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!existingItem || existingItem.store.merchantId !== ctx.merchantId) {
				throw new TRPCError({
					code: "NOT_FOUND",
	...
	delete: storeOwnerProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!item || item.store.merchantId !== ctx.merchantId) {
				throw new TRPCError({
					code: "NOT_FOUND",
					message: "Item not found or does not belong to your store",
				});
