 ...
 * Onboarding Schemas
 *
 * Zod schemas for merchant onboarding API inputs.
 */
...
export const legalEntitySchema = z
	.object({
		...
		legalFormOther: z.string().min(2).max(100).optional().or(z.literal("")),
		companyName: z.string().min(2).max(255),
		representativeName: z.string().min(2).max(255),
		registerCourt: z.string().min(2).max(100).optional().or(z.literal("")),
		registerNumber: z.string().min(2).max(50).optional().or(z.literal("")),
		vatId: z
			.string()
...
/**
 * Signup schema - create new merchant and first store
 */
