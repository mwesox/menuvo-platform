...
- `apps/` contains deployable apps: `api` (Hono + tRPC), `console` and `shop` (Vite + React SPAs), and `business` (marketing site).
- `packages/` holds shared code: `db` (Drizzle schema + client), `trpc` (routers + shared types), `ui` (shadcn-based components).
- `docs/` contains architecture and coding guidelines; read `docs/architecture.md` and `docs/coding-guidelines.md` before larger changes.
...
- Vitest is used in frontend apps; run locally with `cd apps/console && bunx vitest` as needed, or via `bun run test` if your app defines a `test` script.
...
All data access flows through tRPC: SPA → tRPC client → API → PostgreSQL. Keep routes thin, move business logic into `apps/api/src/services/`, and keep shared UI in `packages/ui`.
